...
describe("webviewMessageHandler - deleteCustomMode", () => {
	beforeEach(() => {
		...
		vi.mocked(ensureSettingsDirectoryExists).mockResolvedValue("/mock/global/storage/.adtec")
	})

	it("should delete a project mode and its rules folder", async () => {
		const slug = "test-project-mode"
		const rulesFolderPath = path.join("/mock/workspace", ".adtec", `rules-${slug}`)
	...
	it("should delete a global mode and its rules folder", async () => {
		...
		const rulesFolderPath = path.join(homeDir, ".adtec", `rules-${slug}`)
		...
		const getSkillsForMode = vi.fn().mockReturnValue([
			{
				...
				path: "/mock/.adtec/skills/skill-slug-entry/SKILL.md",
				source: "project",
				modeSlugs: ["code"],
			},
			{
				...
				path: "/mock/.adtec/skills/duplicate-skill/SKILL.md",
				source: "global",
				modeSlugs: ["code"],
			},
			{
				...
				path: "/mock/.adtec/skills/another-skill-slug/SKILL.md",
				source: "global",
				modeSlugs: ["code"],
			},
		])
			...
			expect.arrayContaining([
				{
					name: "skill-slug-entry",
					source: "project",
					filePath: "/mock/.adtec/skills/skill-slug-entry/SKILL.md",
					description: "Primary skill slug",
				},
				{
					name: "another-skill-slug",
		...
		mockGetCommands.mockResolvedValue([
			{
				...
				source: "project",
				filePath: "/mock/workspace/.adtec/commands/deploy.md",
		...
		const getSkillsForMode = vi.fn().mockReturnValue([
			{
				...
				path: "/mock/.adtec/skills/deploy/SKILL.md",
				source: "global",
				modeSlugs: ["code"],
			},
			{
				...
				path: "/mock/.adtec/skills/skill-only/SKILL.md",
				source: "project",
				modeSlugs: ["code"],
			},
		])
		...
		expect(mockClineProvider.postMessageToWebview).toHaveBeenCalledWith({
			...
			commands: expect.arrayContaining([
				...
				},
				{
					name: "skill-only",
					source: "project",
					filePath: "/mock/.adtec/skills/skill-only/SKILL.md",
					description: "Skill-generated command",
				},
			]),
		})
