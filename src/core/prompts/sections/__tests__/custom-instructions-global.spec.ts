...
describe("custom-instructions global .adtec support", () => {
	const mockCwd = "/mock/project"
	const mockHomeDir = "/mock/home"
	const globalRooDir = path.join(mockHomeDir, ".adtec")
	const projectRooDir = path.join(mockCwd, ".adtec")
	...
	beforeEach(() => {
		...
		// getAgentsDirectoriesForCwd returns parent directories (without .adtec)
	...
	describe("loadRuleFiles", () => {
		...
		it("should fall back to legacy .adtecrules file when no .adtec/rules directories exist", async () => {
		...
		it("should return empty string when no rules exist anywhere", async () => {
			...
			mockReadFile
				.mockResolvedValueOnce("") // .adtecrules returns empty (simulating ENOENT caught by safeReadFile)
	...
	describe("addCustomInstructions mode-specific rules", () => {
		it("should load global and project mode-specific rules", async () => {
			...
			mockReadFile
				...
				.mockResolvedValueOnce("") // .adtecrules legacy file (empty)
		...
		it("should fall back to legacy mode-specific files when no mode directories exist", async () => {
			...
			mockReadFile
				.mockResolvedValueOnce("legacy mode rule content") // .adtecrules-code
				.mockResolvedValueOnce("") // AGENTS.md file (empty)
				.mockResolvedValueOnce("") // generic .adtecrules (empty)
				.mockResolvedValueOnce("") // generic .clinerules (empty)

			const result = await addCustomInstructions("", "", mockCwd, mode)

			expect(result).toContain("# Rules from .adtecrules-code:")
