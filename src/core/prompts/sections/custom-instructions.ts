...
export async function loadRuleFiles(cwd: string, enableSubfolderRules: boolean = false): Promise<string> {
	...
	// Check for .adtec/rules/ directories in order (global, project-local, and optionally subfolders)
	...
	// If we found rules in .adtec/rules/ directories, return them
	if (rules.length > 0) {
		return "\n# Rules from .adtec directories:\n\n" + rules.join("\n\n")
	}

	// Fall back to existing behavior for legacy .adtecrules/.clinerules files
	const ruleFiles = [".adtecrules", ".clinerules"]
...
/**
 * Load all AGENTS.md files from project root and optionally subdirectories with .adtec folders
...
async function loadAllAgentRulesFiles(cwd: string, enableSubfolderRules: boolean = false): Promise<string> {
	...
	// When enabled, load from root and all subdirectories with .adtec folders
...
export async function addCustomInstructions(
	...
	if (mode) {
		...
		// Check for .adtec/rules-${mode}/ directories in order (global, project-local, and optionally subfolders)
		for (const rooDir of rooDirectories) {
		...
		// If we found mode-specific rules in .adtec/rules-${mode}/ directories, use them
		if (modeRules.length > 0) {
			modeRuleContent = "\n" + modeRules.join("\n\n")
			usedRuleFile = `rules-${mode} directories`
		} else {
			// Fall back to existing behavior for legacy files
			const rooModeRuleFile = `.adtecrules-${mode}`
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path.join(".adtec", `rules-${mode}`))) {
	...
	// Load from root and optionally subdirectories with .adtec folders based on enableSubfolderRules setting
