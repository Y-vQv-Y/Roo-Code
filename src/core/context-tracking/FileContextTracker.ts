...
// If a user modifies a file outside of ADTEC Code, the context may become stale and need to be updated.
// We do not want ADTEC Code to reload the context every time a file is modified, so we use this class merely
// to inform ADTEC Code that the change has occurred, and tell ADTEC Code to reload the file before making
// any changes to it. This fixes an issue with diff editing, where ADTEC Code was unable to complete a diff edit.

// FileContextTracker
//
// This class is responsible for tracking file operations.
// If the full contents of a file are passed to ADTEC Code via a tool, mention, or edit, the file is marked as active.
// If a file is modified outside of ADTEC Code, we detect and track this change to prevent stale context.
export class FileContextTracker {
	...
	async setupFileWatcher(filePath: string) {
		...
		watcher.onDidChange(() => {
			if (this.recentlyEditedByRoo.has(filePath)) {
				this.recentlyEditedByRoo.delete(filePath) // This was an edit by ADTEC Code, no need to inform ADTEC Code
			} else {
				this.recentlyModifiedFiles.add(filePath) // This was a user edit, we will inform ADTEC Code
	...
	// This is the main entry point for FileContextTracker and is called when a file is passed to ADTEC Code via a tool, mention, or edit.
	...
	public getContextProxy(): ContextProxy | undefined {
		...
		if (!provider) {
			console.error("ADTEC Code provider reference is no longer valid")
			return undefined
		}
	...
	async addFileToFileContextTracker(taskId: string, filePath: string, source: RecordSource) {
		try {
			...
			switch (source) {
				...
				// roo_edited: ADTEC Code has edited the file
				case "roo_edited":
				...
				// read_tool/file_mentioned: ADTEC Code has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.roo_read_date = now
					break
	...
	/**
	 * Gets a list of unique file paths that ADTEC Code has read during this task.
	...
	async getFilesReadByRoo(sinceTimestamp?: number): Promise<string[]> {
		try {
			...
			const readEntries = metadata.files_in_context.filter((entry) => {
				// Only include files that were read by ADTEC Code (not user edits)
			...
			console.error("Failed to get files read by ADTEC Code:", error)
			return []
		}
	}
	...
	// Marks a file as edited by ADTEC Code to prevent false positives in file watchers
