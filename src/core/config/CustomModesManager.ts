...
export class CustomModesManager {
	...
	private parseYamlSafely(content: string, filePath: string): any {
		...
		try {
			...
			// For non-.adtecmodes files, just log and return empty object
			const errorMsg = yamlError instanceof Error ? yamlError.message : String(yamlError)
			console.error(`[CustomModesManager] Failed to parse YAML from ${filePath}:`, errorMsg)
			return {}
		}
	...
	private async watchCustomModesFiles(): Promise<void> {
		...
		// Watch .adtecmodes file - watch the path even if it doesn't exist yet
		const workspaceFolders = vscode.workspace.workspaceFolders
		if (workspaceFolders && workspaceFolders.length > 0) {
			const workspaceRoot = getWorkspacePath()
			const projectModesPath = path.join(workspaceRoot, PROJECT_MODES_FILENAME)
			...
			const handleProjectModesChange = async () => {
				try {
					...
					console.error(`[CustomModesManager] Error handling .adtecmodes file change:`, error)
				}
			}
			...
			this.disposables.push(
				projectModesWatcher.onDidDelete(async () => {
					...
					try {
						...
						console.error(`[CustomModesManager] Error handling .adtecmodes file deletion:`, error)
					}
				}),
			)
	...
	private async deleteRulesFolder(slug: string, mode: ModeConfig): Promise<void> {
		try {
			...
			if (scope === "project") {
				...
				if (workspacePath) {
					rulesFolderPath = path.join(workspacePath, ".adtec", `rules-${slug}`)
				} else {
				...
				rulesFolderPath = path.join(homeDir, ".adtec", `rules-${slug}`)
			}
	...
	/**
	 * Checks if a mode has associated rules files in the .adtec/rules-{slug}/ directory
	...
	public async checkRulesDirectoryHasContent(slug: string): Promise<boolean> {
		try {
			...
			if (!mode) {
				// If not in custom modes, check if it's in .adtecmodes (project-specific)
				const workspacePath = getWorkspacePath()
				if (!workspacePath) {
					return false
				}
				...
				try {
					...
					if (projectModesExist) {
						...
						return false // No .adtecmodes file and not in custom modes
					}
				} catch (error) {
					return false // Cannot read .adtecmodes and not in custom modes
				}
			}
			...
			if (isGlobalMode) {
				// For global modes, check in global .adtec directory
				const globalRooDir = getGlobalRooDirectory()
				modeRulesDir = path.join(globalRooDir, `rules-${slug}`)
			} else {
				// For project modes, check in workspace .adtec directory
				const workspacePath = getWorkspacePath()
				if (!workspacePath) {
					return false
				}
				modeRulesDir = path.join(workspacePath, ".adtec", `rules-${slug}`)
			}
	...
	public async exportModeWithRules(slug: string, customPrompts?: PromptComponent): Promise<ExportResult> {
		try {
			...
			let baseDir: string
			if (isGlobalMode) {
				// For global modes, use the global .adtec directory
			...
			// Check for .adtec/rules-{slug}/ directory (or rules-{slug}/ for global)
			const modeRulesDir = isGlobalMode
				? path.join(baseDir, `rules-${slug}`)
				: path.join(baseDir, ".adtec", `rules-${slug}`)
	...
	private async importRulesFiles(
		...
		if (source === "global") {
			...
			const workspacePath = getWorkspacePath()
			baseDir = path.join(workspacePath, ".adtec")
			rulesFolderPath = path.join(baseDir, `rules-${importMode.slug}`)
		}
