	...
	(klass, prefix) => {
		...
		describe(`${klass.name}#hasNestedGitRepositories`, () => {
			it("throws error when nested git repositories are detected during initialization", async () => {
				...
				await mainGit.init()
				await mainGit.addConfig("user.name", "ADTEC Code")
				await mainGit.addConfig("user.email", "developer@example.com")
				...
				await nestedGit.addConfig("user.name", "ADTEC Code")
				await nestedGit.addConfig("user.email", "developer@example.com")
		...
		describe(`${klass.name}#saveCheckpoint with allowEmpty option`, () => {
			...
			it("isolates checkpoint operations from GIT_DIR environment variable", async () => {
				...
				// In the real-world Dev Container scenario, GIT_DIR is set BEFORE ADTEC Code starts,
