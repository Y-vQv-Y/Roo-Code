...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("adtec-code", context)
			...
			setRuntimeConfig("adtec-code", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("adtec-code", "testSetting", "testValue")

			expect(getRuntimeConfig("adtec-code.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("adtec-code", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("adtec-code.setting1")).toBe("value1")
			expect(getRuntimeConfig("adtec-code.setting2")).toBe(42)
			expect(getRuntimeConfig("adtec-code.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("adtec-code.defined")).toBe("value")
			expect(getRuntimeConfig("adtec-code.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("adtec-code", "setting1", "value1")
			setRuntimeConfig("adtec-code", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("adtec-code.setting1")).toBeUndefined()
			expect(getRuntimeConfig("adtec-code.setting2")).toBeUndefined()
		})
