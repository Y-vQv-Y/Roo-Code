...
# ADTEC Code 3.16 Release Notes (2025-05-12)
...
- **Automatic Model Discovery**: ADTEC Code now automatically fetches and lists available models from your LiteLLM server. This means users no longer need to manually configure each LiteLLM model within ADTEC Code, streamlining setup and making it easier to switch between models served by LiteLLM.
...
- You can now toggle the `terminal.integrated.inheritEnv` VSCode setting directly for the ADTEC Code settings (thanks KJ7LNW!)
