---
description: ADTEC Code 3.14 brings Gemini caching, Boomerang Orchestrator mode, improved file editing tools, and a new minimalist UI design.
keywords:
    - ADTEC Code
    - v3.14
    - release notes
...
- Automatically close files opened by edit tools (`apply_diff`, `insert_content`, `search_and_replace`, `write_to_file`) after changes are approved. This prevents cluttering the editor with files opened by ADTEC Code and helps clarify context by only showing files intentionally opened by the user.
...
- **Override Warning:** ADTEC Code now displays a warning indicator in the chat input when a system prompt override is active for the current mode.
...
- Corrected word wrapping in ADTEC Code message titles (thanks zhangtony239!).
