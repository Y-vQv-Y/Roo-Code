---
description: ADTEC Code 3.39 includes task-level provider profile persistence, improved multimodal workflows, new provider/model options, and reliability fixes across tool calling.
keywords:
  - ADTEC Code 3.39
  - new features
  - bug fixes
...
# ADTEC Code 3.39 Release Notes (Combined)

ADTEC Code 3.39 includes task-level provider profile persistence, improved multimodal workflows, new provider/model options, and reliability fixes across tool calling.

## Features

* **Task-level API configuration profiles**: ADTEC Code can persist the selected provider profile per task so follow-up sessions keep using the same provider/model settings.
* **Image file @mentions**: You can mention images in your workspace to give the model visual context directly from chat.
* **Headless ADTEC Code (CLI + VS Code shim)**: Run ADTEC Code in a headless environment with a basic CLI, plus an installer and an option to use the CLI for evals.
* **Debug proxy routing**: Adds debug-mode proxy routing to help troubleshoot API calls.

## QOL Improvements

* **BRRR naming update**: Renames YOLO to BRRR.
* **Smarter file @mention search**: Filters file @mention results using `.adtecignore`.
...
* Renames ADTEC Code Cloud Provider to **ADTEC Code Router** for clearer branding.
* Updates ADTEC Code Router service name and related wording so the UI uses consistent naming.
...
* Adds an interactive TUI mode to the ADTEC Code CLI.

### 3.39.2 (2026-01-09)

* Adds an **Enable debug mode** toggle in ADTEC Code settings (About ADTEC Code) so you can turn debug logging on/off from the extension UI.
...
* Updates ADTEC Code CLI defaults by switching the default model to `anthropic/claude-opus-4.5`.
* Updates ADTEC Code Cloud’s Terms of Service (effective 2026-01-09).
