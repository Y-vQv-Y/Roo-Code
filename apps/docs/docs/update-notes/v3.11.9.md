...
# ADTEC Code 3.11.9 Release Notes (2025-04-07)
...
## Custom ADTEC Code Instructions (`.adtec/` Directories)

- Introduced support for placing multiple instruction files within `.adtec/rules/` (for workspace-wide) and `.adtec/rules-{modeSlug}/` (for mode-specific) directories. ADTEC Code recursively reads all files within these directories, appending them to the system prompt in **alphabetical order** by filename.
- This allows for better organization and management of complex instruction sets compared to single files.
- This directory-based method now takes precedence over the single `.adtecrules` or `.adtecrules-{modeSlug}` files if the corresponding directory exists and contains files. (Thanks upamune!)
...
- Added command to focus ADTEC Code input field (thanks axkirillov!).
