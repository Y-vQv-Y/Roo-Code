---
description: ADTEC Code 3.3 brings VS Code code actions, mode switching, Debug mode, experimental checkpoints, @terminal mentions, enhanced Markdown editing, and extensive provider updates.
keywords:
    - ADTEC Code 3.3
    - code actions
    - debug mode
...
# ADTEC Code 3.3 Release Notes (2025-02-27)
...
- **Project-Specific Custom Modes:** Support for project-specific custom modes in a `.adtecmodes` file.
- **`@terminal` Context Mention:** Added the ability to mention `@terminal` to pull terminal output into the chat context. (thanks ADTEC Code!)
...
- **New Task Tool:** Added a `new_task` tool that allows ADTEC Code to start new tasks with an initial message and mode.
- **Checkpoints (Experimental & Improved):** Introduced experimental checkpoints using a shadow git repository, with subsequent fixes for Windows file locks and UI. (thanks ADTEC Code!)
...
- Fixed `.adtecmodes` auto-creation.
