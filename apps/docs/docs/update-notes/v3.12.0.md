---
description: ADTEC Code 3.12 adds xAI provider support, profile-specific diff settings, keyboard shortcuts for input, and search in dropdowns.
keywords:
    - ADTEC Code
    - v3.12
    - release notes
...
# ADTEC Code 3.12 Release Notes (2025-04-16)
...
- Added [xAI provider](/providers/xai) and exposed reasoning effort options for Grok on OpenRouter. (thanks ADTEC Code!)
...
Added the `adtec-code.acceptInput` command to allow users to accept input or suggestions using keyboard shortcuts instead of mouse clicks. (thanks axkirillov!) This feature:
