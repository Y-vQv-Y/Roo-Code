---
description: Generate or edit images using AI models through the generate_image tool in ADTEC Code.
keywords:
    - generate_image
    - AI images
    - image generation
...
The `generate_image` tool creates new images from text prompts or modifies existing images using AI models. It supports two providers: **OpenRouter** and the **ADTEC Code provider**. This experimental feature enables visual content generation and transformation within your development workflow.
...
- Powered by **OpenRouter** or the **ADTEC Code provider** for access to various AI models
...
- Requires OpenRouter or ADTEC Code provider API configuration
...
3. **API Request**: Sends request to the configured provider (OpenRouter or ADTEC Code) with prompt and optional input image.
