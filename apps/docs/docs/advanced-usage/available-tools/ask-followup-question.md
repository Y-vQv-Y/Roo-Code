---
description: Enable interactive communication in ADTEC Code with the ask_followup_question tool for gathering clarification and user preferences.
keywords:
...
This tool creates a conversational interface between ADTEC Code and the user, allowing for gathering clarification, additional details, or user preferences when facing ambiguities or decision points. Each question can include suggested responses to streamline the interaction.
...
- When ADTEC Code needs to choose between multiple valid implementation approaches
- When technical details or preferences are required to proceed
- When ADTEC Code encounters ambiguities that need resolution
...
1. **Information Gap Recognition**: ADTEC Code identifies missing information needed to proceed
2. **Specific Question Creation**: ADTEC Code formulates a clear, targeted question
3. **Suggestion Development**: ADTEC Code creates relevant suggested answers (optional but recommended)
...
10. **Task Continuation**: ADTEC Code proceeds with the task using the new information

---

## Examples When Used

- When developing a web application, ADTEC Code might ask about preferred styling frameworks (Bootstrap, Tailwind, custom CSS)
- When creating an API, ADTEC Code might ask about authentication methods (JWT, OAuth, API keys)
- When refactoring code, ADTEC Code might ask about prioritizing performance vs. readability
- When setting up a database, ADTEC Code might ask about specific schema design preferences
- When creating a custom feature, ADTEC Code might ask about specific behavior expectations
- When troubleshooting errors, ADTEC Code might ask about specific environment details
