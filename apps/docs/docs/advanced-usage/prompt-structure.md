---
description: Understand the technical structure of prompts in ADTEC Code. Learn how messages are constructed, system prompts work, and optimize your interactions.
keywords:
    - prompt structure
    - system prompt
    - message flow
...
This page explains the technical structure of prompts in ADTEC Code - how messages are constructed and sent to the Large Language Model (LLM).

---

## Core Message Types

ADTEC Code uses three primary message types when communicating with LLMs:

- **System Prompt**: The initial instructions that define ADTEC Code's capabilities, persona, and operational rules
- **User Messages**: Content sent by you (the user) to ADTEC Code
...
Understanding these message types helps you work more effectively with ADTEC Code and can be valuable for troubleshooting or advanced customization.

---

## System Prompt

The system prompt is the foundation of ADTEC Code's behavior. It contains:
...
- **Capabilities**: Description of what ADTEC Code can do in the current environment
...
The system prompt is generated dynamically each time you interact with ADTEC Code, adapting to your current mode, available tools, and custom settings.

---

## User Messages

User messages contain your direct inputs to ADTEC Code, plus additional contextual information:
...
This automatic context enrichment helps ADTEC Code understand your workspace without requiring you to explicitly describe it.
...
1. **Initial Setup**: ADTEC Code generates the system prompt based on your selected mode and configuration
...
5. **Tool Execution**: If the LLM requests a tool, ADTEC Code executes it and provides the result
...
Internally, ADTEC Code's prompt construction is handled by several components:
...
- **Message Transformation**: Provider-specific transformers convert ADTEC Code's internal message format to the format required by each LLM API

---

## Support Prompts

Alongside the main chat flow, ADTEC Code uses specialized templates for specific code actions:
...
- **Write Better Prompts**: Knowing what context ADTEC Code already has helps you avoid redundant information
...
This technical foundation powers all of ADTEC Code's capabilities, enabling it to understand your requests and effectively utilize available tools to complete tasks.
