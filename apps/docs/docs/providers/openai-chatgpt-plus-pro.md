...
description: Use OpenAI models in ADTEC Code with your ChatGPT Plus/Pro subscription (OAuth sign-in, no API key).
keywords:
    - OpenAI Codex
    - ChatGPT Plus
    - ChatGPT Pro
...
## Quickstart: Connect your subscription to ADTEC Code
...
5. Back in ADTEC Code settings, pick a model from the dropdown.
6. Save.
...
- **You can't use arbitrary OpenAI API models.** This provider only exposes the models listed in ADTEC Code's Codex model catalog.
- **You can't export/migrate your sign-in state with settings export.** OAuth tokens are stored in VS Code SecretStorage, which isn't included in ADTEC Code's settings export.
