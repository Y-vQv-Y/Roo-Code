...
description: Learn how to set up and use LM Studio with ADTEC Code for running local language models with OpenAI-compatible API support.
keywords:
    - LM Studio
    - local models
    - ADTEC Code
...
# Using LM Studio With ADTEC Code

ADTEC Code supports running models locally using LM Studio. LM Studio provides a user-friendly interface for downloading, configuring, and running local language models. It also includes a built-in local inference server that emulates the OpenAI API, making it easy to integrate with ADTEC Code.
...
4.  **(Optional) Base URL:** By default, ADTEC Code will connect to LM Studio at `http://localhost:1234`. If you've configured LM Studio to use a different address or port, enter the full URL here.
...
- **Local Server:** The LM Studio local server must be running for ADTEC Code to connect to it.
...
- **Token Tracking:** ADTEC Code tracks token usage for models run via LM Studio, helping you monitor consumption.
- **Reasoning Support:** For models that support it, ADTEC Code can parse "think" tags or similar reasoning indicators in LM Studio responses, offering more insight into the model's process.
