...
description: Use ADTEC Code with any OpenAI-compatible API provider including Perplexity, Together AI, Anyscale, and custom endpoints.
keywords:
    - OpenAI compatible
    - ADTEC Code
    - API integration
...
# Using OpenAI Compatible Providers With ADTEC Code

ADTEC Code supports a wide range of AI model providers that offer APIs compatible with the OpenAI API standard. This means you can use models from providers _other than_ OpenAI, while still using a familiar API interface. This includes providers like:
...
You'll find these settings in the ADTEC Code settings panel (click the <Codicon name="gear" /> icon):
...
ADTEC Code uses **native tool calling** exclusively. This is the only supported tool protocol -- there is no XML-based fallback.
...
- **Tool arguments** are streamed incrementally, which reduces latency between the model deciding to use a tool and ADTEC Code executing it.

### Requirements

For native tool calling to work, the model you select **must** support OpenAI-compatible tool calling. If a model does not support native tool calling, it cannot be used with ADTEC Code.
...
When the model decides to use `read_file`, ADTEC Code surfaces **streamed tool events** in the task timeline:
...
- **Model support:** Not all models support native tool calling. If a model does not support tools, it cannot be used with ADTEC Code. Check your provider's documentation to confirm tool-calling support for the model you want to use.
...
For a deeper overview of how tools work in ADTEC Code in general, see the [Tool Use Overview](/advanced-usage/available-tools/tool-use-overview).
...
- **Tool-calling errors:** ADTEC Code requires native tool calling. If your model does not support it, you need to switch to a model that does. Check your provider's documentation for tool-calling compatibility.
- **Unexpected Results:** If you're getting unexpected results, try a different model.

By using an OpenAI-compatible provider, you can leverage the flexibility of ADTEC Code with a wider range of AI models. Remember to always consult your provider's documentation for the most accurate and up-to-date information.
