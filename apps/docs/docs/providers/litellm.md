...
description: Access over 100 LLMs through LiteLLM's unified OpenAI-compatible API in ADTEC Code. Simplify multi-model management and reduce costs.
keywords:
    - litellm
    - ADTEC Code
    - api provider
...
- **Simplified Configuration:** Manage credentials and model configurations in one place (your LiteLLM server) and let ADTEC Code connect to it.
...
To use LiteLLM with ADTEC Code, you first need to set up and run a LiteLLM server.
...
Once your LiteLLM server is running, you have two options for configuring it in ADTEC Code:
...
4.  **Enter API Key (Optional):**
    ...
    - If your LiteLLM server doesn't require an API key, ADTEC Code will use a default dummy key (`"dummy-key"`), which should work fine.
5.  **Select Model:**
    - ADTEC Code will attempt to fetch the list of available models from your LiteLLM server by querying the `${baseUrl}/v1/model/info` endpoint.
    ...
    - If no model is selected, ADTEC Code will use a default model. Ensure you have configured at least one model on your LiteLLM server.
...
<img src="/img/litellm/litellm.png" alt="ADTEC Code LiteLLM Provider Settings" width="600" />

---

## How ADTEC Code Fetches and Interprets Model Information

When you configure the LiteLLM provider, ADTEC Code interacts with your LiteLLM server to get details about the available models:

- **Model Discovery:** ADTEC Code makes a GET request to `${baseUrl}/v1/model/info` on your LiteLLM server. If an API key is provided in ADTEC Code's settings, it's included in the `Authorization: Bearer ${apiKey}` header.
- **Model Properties:** For each model reported by your LiteLLM server, ADTEC Code extracts and interprets the following:
    ...
    - `supportsComputerUse`: This flag is set to `true` if the underlying model identifier matches one of the Anthropic models predefined in ADTEC Code as suitable for "computer use" (see `COMPUTER_USE_MODELS` in technical details).

ADTEC Code uses default values for some of these properties if they are not explicitly provided by your LiteLLM server's `/model/info` endpoint for a given model. The defaults are:
...
- **LiteLLM Server is Key:** The primary configuration for models, API keys for downstream providers (like OpenAI, Anthropic), and other advanced features are managed on your LiteLLM server. ADTEC Code acts as a client to this server.
...
- **Model Availability:** The models available in ADTEC Code's "Model" dropdown depend entirely on what your LiteLLM server exposes through its `/v1/model/info` endpoint.
- **Network Accessibility:** Ensure your LiteLLM server is running and accessible from the machine where VS Code and ADTEC Code are running (e.g., check firewall rules if not on `localhost`).
- **Troubleshooting:** If models aren't appearing or requests fail:
    ...
    - Ensure the Base URL in ADTEC Code settings matches your LiteLLM server's address.
    - Confirm any API key required by your LiteLLM server is correctly entered in ADTEC Code.
- **Computer Use Models:** The `supportsComputerUse` flag in ADTEC Code is primarily relevant for certain Anthropic models known to perform well with tool-use and function-calling tasks. If you are routing other models through LiteLLM, this flag might not be automatically set unless the underlying model ID matches the specific Anthropic ones ADTEC Code recognizes.

By leveraging LiteLLM, you can significantly expand the range of models accessible to ADTEC Code while centralizing their management.
