---
description: Learn how to configure auto-approval settings in ADTEC Code to speed up repetitive tasks while maintaining control over sensitive operations.
keywords:
  - auto-approval
  - ADTEC Code automation
...
**Command Palette:** `adtec-code.toggleAutoApprove`
...
2. Type "Toggle Auto-Approve" or search for `adtec-code.toggleAutoApprove`
...
|------------|--------------|------------|
| **Read files and directories** | Lets ADTEC Code access files without asking | Medium |
| **Edit files** | Lets ADTEC Code modify files without asking | **High** |
...
| **Use MCP servers** | Lets ADTEC Code use configured MCP services | Medium-High |
| **Switch modes** | Changes between ADTEC Code modes automatically | Low |
...
> **Allow ADTEC Code to automatically perform operations without requiring approval. Enable these settings only if you fully trust the AI and understand the associated security risks.**
...
- After auto‑writes, ADTEC Code waits using the global write‑delay control under Settings → Context Management → Diagnostics.
...
**Description:** "When enabled, ADTEC Code will automatically view directory contents and read files without requiring you to click the Approve button."

**Additional option:**
- **Include files outside workspace:** Allow ADTEC Code to read files outside the current workspace directory

**Risk level:** Medium

While this setting only allows reading files (not modifying them), it could potentially expose sensitive data. Still recommended as a starting point for most users, but be mindful of what files ADTEC Code can access.

#### Workspace Boundary Protection

By default, ADTEC Code can only read files within your current workspace directory. The "Include files outside workspace" option extends read access beyond the workspace boundary. Consider the security implications:

- **Default (unchecked)**: ADTEC Code can only read files in your project directory
- **Enabled**: ADTEC Code can read any file on your system that you have access to
- **Recommendation**: Keep disabled unless you specifically need ADTEC Code to access external files
:::
...
**Additional options:**
- **Include files outside workspace:** Allow ADTEC Code to modify files outside the current workspace directory
- **Include protected files:** Allow ADTEC Code to modify files protected by `.adtec/` and `.adtecignore` directory

**Risk level:** High

This setting allows ADTEC Code to modify your files without confirmation. The delay timer is crucial:
...
1. **Workspace Boundary Protection**: By default, ADTEC Code can only modify files within your current workspace. Enable "Include files outside workspace" with extreme caution.

2. **Protected Files**: ADTEC Code blocks modification of:
   - Files in the `.adtec/` directory
   - The `.adtecignore` file
...
*VSCode Problems pane that ADTEC Code checks during the write delay*

When you enable auto-approval for writing files, ADTEC Code uses the global write-delay configured under Settings → Context Management → Diagnostics. The delay timer works with VSCode's Problems pane:

1. ADTEC Code makes a change to your file
2. VSCode's diagnostic tools analyze the change
3. The Problems pane updates with any errors or warnings
4. ADTEC Code notices these issues before continuing
...
**Permission Storage:** Your MCP tool permissions are saved and will be remembered next time you use ADTEC Code.
...
Allows ADTEC Code to change between different modes (Code, Architect, etc.) without asking for permission. This primarily affects the AI's behavior rather than system access. Auto-approval also covers switching between modes and creating new modes when this permission is enabled.
:::
...
Enables ADTEC Code to create and complete subtasks automatically. This relates to workflow organization rather than system access.
:::
...
```json
{
  "adtec-code.allowedCommands": ["git", "npm run", "echo"],
  "adtec-code.deniedCommands": ["git push", "npm publish", "rm", "sudo", "*"]
}
```
...
**Description:** Automatically selects the first AI-suggested answer for a follow-up question after a configurable timeout. This speeds up your workflow by letting ADTEC Code proceed without manual intervention.
...
**Use cases:**
- Overnight runs where you want ADTEC Code to continue working
