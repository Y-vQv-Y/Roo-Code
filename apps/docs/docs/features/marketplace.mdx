---
description: Discover and install MCP servers and custom modes from the ADTEC Code Marketplace to extend your AI coding assistant's capabilities.
keywords:
...
The ADTEC Code Marketplace is a central hub for discovering and installing extensions, known as MCPs (Model Context Protocol) and Modes. It allows you to easily extend the functionality of ADTEC Code to fit your specific needs and workflows.
...
The ADTEC Code Marketplace is available directly within the ADTEC Code extension in VS Code. Access it by clicking the marketplace icon <Codicon name="extensions" /> in the top menu bar.
...
MCPs allow ADTEC Code to connect to and interact with various AI models, APIs, and other external tools. By installing an MCP, you can configure ADTEC Code to use different language models (like those from OpenAI, Anthropic, or others) or to integrate with other services. Learn more about [What is MCP?](/features/mcp/what-is-mcp) and [how to use MCP in ADTEC Code](/features/mcp/using-mcp-in-adtec).

<img src="/img/marketplace/marketplace-1.png" alt="MCP tab showing available MCPs with install and remove buttons" width="500" />

### Modes
Modes are custom sets of instructions and rules that tailor ADTEC Code's behavior for specific tasks. For example, you might find a "React Component" mode that is optimized for creating React components, or a "Documentation Writer" mode for writing technical documentation. Learn more about [using modes](/basic-usage/using-modes) and [creating custom modes](/features/custom-modes).
...
- **Configuration File**:
    - **MCPs**: [`.adtec/mcp.json`](/features/mcp/using-mcp-in-adtec#configuring-mcp-servers) in the root of your project.
    - **Modes**: [`.adtecmodes`](/features/custom-modes#3-manual-configuration-yaml--json) in the root of your project.
...
- **Configuration File**:
    - **MCPs**: [`mcp_settings.json`](/features/mcp/using-mcp-in-adtec#configuring-mcp-servers) in the ADTEC Code extension's global settings directory.
    - **Modes**: [`custom_modes.yaml`](/features/custom-modes) in the ADTEC Code extension's global settings directory.
...
5. ADTEC Code automatically adds the item to the appropriate configuration file. If the file doesn't exist, ADTEC Code will create it for you. The file is then opened for your review.
...
4. ADTEC Code removes the item from the corresponding configuration file.
...
### Installation Errors
- **Invalid YAML/JSON**: The configuration file (`.adtecmodes`, `.adtec/mcp.json`, etc.) has a syntax error. To prevent data loss, ADTEC Code will not modify a corrupted configuration file. Please fix the syntax error before installing or removing items.
- **File Not Found**: Rare error - ADTEC Code automatically creates necessary configuration files.
...
4. **Check ADTEC Code logs**: Look for error messages in the ADTEC Code output panel.
...
- [Using MCP in ADTEC Code](/features/mcp/using-mcp-in-adtec) - Detailed configuration and usage guide
