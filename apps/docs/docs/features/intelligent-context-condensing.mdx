...
As your conversation with ADTEC Code grows, it might approach the context window limit of the underlying AI model. When this happens, older messages would typically be removed to make space. Intelligent Context Condensing aims to prevent this abrupt loss by:
...
1.  Open ADTEC Code settings (<Codicon name="gear" /> icon in the top right corner of the ADTEC Code panel).
...
3.  Configure the available options:
    ...
    - **Custom Context Condensing Prompt**: Customize the prompt ADTEC Code uses when condensing (found in "Context Management" settings)
...
ADTEC Code provides several ways to control and understand the Intelligent Context Condensing feature:

#### Controlling context condensing
*   **Automatic Threshold:** The threshold slider in "Context" settings allows you to define a percentage (e.g., 80%) of context window usage. ADTEC Code will attempt to condense the context automatically when the conversation reaches this level of capacity.
...
*   **Context Condensing Metrics:** When context condensing occurs, ADTEC Code displays:
...
*Why this view matters: it’s your audit trail for what changed, how much it cost, and what ADTEC Code will carry forward.*
...
1. Go to ADTEC Code settings (<Codicon name="gear" /> icon)
2. Open **Context Management**
3. Find the **Custom Context Condensing Prompt** editor
4. Enter your custom prompt that instructs ADTEC Code on what must be preserved
...
When ADTEC Code encounters context window limit errors, it now automatically recovers to keep your work flowing:

#### How error recovery works

1. **Error Detection**: ADTEC Code detects context window errors from multiple providers (OpenAI, Anthropic, and others)
2. **Automatic Truncation**: The system automatically reduces the context by 25%
3. **Retry Mechanism**: After truncation, ADTEC Code retries your request (up to the built-in retry limit)
4. **Continuation**: ADTEC Code retries without manual intervention
...
#### Token counting
ADTEC Code uses a sophisticated token counting system that:
...
- If you had a legacy `customCondensingPrompt`, ADTEC Code migrates it into this location automatically.
- **Reset** clears the override so ADTEC Code falls back to the built-in default condense prompt.
