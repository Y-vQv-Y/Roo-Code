---
description: Learn how to create reusable skills that package task-specific instructions, making ADTEC Code more effective at specialized workflows.
keywords:
  - skills
...
Skills package task-specific instructions that ADTEC Code loads on-demand when your request matches the skill's purpose. Unlike custom instructions that apply to everything, skills activate only when needed—making ADTEC Code more effective at specialized tasks without cluttering the base prompt.
...
**Skills solve this**: Create a skill for PDF processing, and ADTEC Code only loads those instructions when you actually ask to work with PDFs. This keeps the system prompt focused and gives ADTEC Code deep expertise in specific domains without affecting unrelated tasks.
...
- **Team Sharing**: Version-control project skills in `.adtec/skills/` for consistent team workflows
- **Personal Library**: Build a global skills library in `~/.adtec/skills/` that works across all projects
...
**Level 1: Discovery** - ADTEC Code reads each `SKILL.md` file and parses its frontmatter to extract `name` and `description`. Only this metadata is stored for matching—the full content isn't held in memory until needed.

**Level 2: Instructions** - When your request matches a skill's description, ADTEC Code uses [`read_file`](/advanced-usage/available-tools/read-file) to load the full SKILL.md instructions into context.

**Level 3: Resources** - The prompt tells ADTEC Code it may access bundled files (scripts, templates, references) alongside the skill. There's no separate resource manifest—ADTEC Code discovers these files on-demand when the instructions reference them.

This architecture means skills remain dormant until activated—they don't bloat your base prompt. You can install many skills, and ADTEC Code loads only what's relevant for each task.
...
```bash
# Linux/macOS — ADTEC Code-specific (higher priority)
~/.adtec/skills/{skill-name}/SKILL.md

# Linux/macOS — cross-agent (shared with other agent tools)
~/.agents/skills/{skill-name}/SKILL.md

# Windows
%USERPROFILE%\.adtec\skills\{skill-name}\SKILL.md
%USERPROFILE%\.agents\skills\{skill-name}\SKILL.md
```

**Project skills** (specific to current workspace):
```bash
# ADTEC Code-specific (higher priority)
<project-root>/.adtec/skills/{skill-name}/SKILL.md
...
mkdir -p ~/.adtec/skills/pdf-processing
touch ~/.adtec/skills/pdf-processing/SKILL.md
```
...
- The description tells ADTEC Code when to use this skill—be specific

#### 4. Test the skill

Ask ADTEC Code something matching the description:
```
"Can you help me extract tables from this PDF file?"
```

ADTEC Code should recognize the request matches your skill description, load the SKILL.md file, and follow its instructions.
...
```
~/.adtec/skills/                    # Global ADTEC Code-specific skills (high priority)
├── pdf-processing/
│   ├── SKILL.md                 # Required
...
.adtec/skills/                      # Project ADTEC Code-specific skills (override global)
└── custom-pdf-workflow/
    └── SKILL.md
...
```
~/.adtec/skills-code/              # Only in Code mode (ADTEC Code-specific, global)
└── refactoring-patterns/
    └── SKILL.md

.adtec/skills-architect/           # Only in Architect mode (ADTEC Code-specific, project)
...
When skills with the same name exist in multiple locations, this priority applies (highest to lowest). `.adtec/` paths always take precedence over `.agents/` paths at the same project level:

1. **Project `.adtec` mode-specific** (`.adtec/skills-code/my-skill/`) — highest priority
2. **Project `.adtec` generic** (`.adtec/skills/my-skill/`)
3. **Project `.agents` mode-specific** (`.agents/skills-code/my-skill/`)
4. **Project `.agents` generic** (`.agents/skills/my-skill/`)
5. **Global `.adtec` mode-specific** (`~/.adtec/skills-code/my-skill/`)
6. **Global `.adtec` generic** (`~/.adtec/skills/my-skill/`)
7. **Global `.agents` mode-specific** (`~/.agents/skills-code/my-skill/`)
8. **Global `.agents` generic** (`~/.agents/skills/my-skill/`) — lowest priority

A **project** skill always overrides a **global** skill of the same name. Within the same project level, `.adtec/` overrides `.agents/`.
...
ADTEC Code automatically discovers skills:
...
ln -s /shared/company-skills ~/.adtec/skills/company-standards
```
...
**Symptom**: ADTEC Code doesn't use your skill even when you request something matching the description.
...
4. **Description too vague**: Make descriptions specific so ADTEC Code can match them to requests
   ```markdown
   # ✗ Vague
...
**Symptom**: ADTEC Code reads the skill but doesn't follow instructions.
...
**Symptom**: Unclear which skill ADTEC Code will use when multiple might match.
...
**Solution**: Place skills in `.adtec/skills/` within your project and commit to version control. Each team member gets the same skills automatically.
...
ADTEC Code skills follow the [Agent Skills](https://agentskills.io/) format for skill packaging and metadata. Skills are **instruction packages with optional bundled files**—they don't register new executable tools.
...
#### ADTEC Code-specific enhancements

ADTEC Code adds mode-specific targeting and cross-agent compatibility beyond the base format:

- **ADTEC Code-specific locations**: `.adtec/skills/`, `~/.adtec/skills/` (higher priority)
- **Cross-agent locations**: `.agents/skills/`, `~/.agents/skills/` (shared with other agent tools)
- **Mode-specific directories**: `skills-{mode}/` (e.g., `skills-code/`, `skills-architect/`) in both `.adtec/` and `.agents/` paths enable mode targeting
