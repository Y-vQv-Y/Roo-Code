---
description: Learn how to create custom modes in ADTEC Code to tailor AI behavior for specific tasks. Configure tool access, file permissions, and specialized instructions.
keywords:
  - custom modes
  - ADTEC Code customization
...
ADTEC Code allows you to create **custom modes** to tailor ADTEC Code's behavior to specific tasks or workflows. Custom modes can be either **global** (available across all projects) or **project-specific** (defined within a single project).

:::tip Sticky Models for Efficient Workflow
Each mode—including custom ones—features **Sticky Models**. This means ADTEC Code automatically remembers and selects the last model you used with a particular mode. This lets you assign different preferred models to different tasks without constant reconfiguration, as ADTEC Code switches between models when you change modes.
:::

:::info Discover Modes
Looking for ready-to-use custom modes? Visit the [ADTEC Code Marketplace](/features/marketplace) to browse and install modes with a single click. The marketplace offers specialized modes for various tasks like React development, documentation writing, testing, and more.
:::
...
<img src="/img/custom-modes/custom-modes-3.png" alt="Overview of custom modes interface" width="500" />
    ...
    *ADTEC Code's interface for creating and managing custom modes.*

---

## What's Included in a Custom Mode?

Custom modes are defined by several key properties. Understanding these concepts will help you tailor ADTEC Code's behavior effectively.

| UI Field / YAML Property                 | Conceptual Description                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Slug (`slug`)                            | A **unique internal identifier** for the mode. It's used by ADTEC Code to reference the mode, especially for associating [mode-specific instruction files](#mode-specific-instructions-via-filesdirectories).                                                                                                                                                                                                   |
| Name (`name`)                            | The **display name** for the mode as it appears in the ADTEC Code user interface. This should be human-readable and descriptive.                                                                                                                                                                                                                                                                               |
...
| Role Definition (`roleDefinition`)       | Defines the **core identity and expertise** of the mode. This text is placed at the beginning of the system prompt.<br />- Its primary function is to define ADTEC Code's personality and behavior when this mode is active.<br />- With the introduction of the `description` field, the `roleDefinition` should provide a detailed description of the mode's identity, while the `description` field handles the short summary for the UI.<br />- The `whenToUse` property now takes precedence for summarization in automated contexts like task orchestration. |
| Available Tools (`groups`)               | Defines the **allowed toolsets and file access permissions** for the mode.<br />- In the UI, this corresponds to selecting which general categories of tools (like reading files, editing files, browsing, or executing commands) the mode can use.<br />- The UI shows which files can be edited in the 'Allowed files' section under each mode.<br />- File type restrictions for the "edit" group are typically managed via manual YAML/JSON configuration or by asking ADTEC Code to set them up, as detailed in the [Property Details for `groups`](#groups). |
| When to Use (optional) (`whenToUse`)     | (Optional) Provides **guidance for ADTEC Code's automated decision-making**, particularly for mode selection and task orchestration.<br />- This text is used by ADTEC Code, particularly the [`🪃 Orchestrator`](/basic-usage/using-modes#orchestrator-mode-aka-boomerang-mode) mode, for [orchestrating tasks](/features/boomerang-tasks) (e.g., via the [`new_task`](/advanced-usage/available-tools/new-task) tool).<br />- It also helps ADTEC Code decide which mode is appropriate when [switching modes](/basic-usage/using-modes#switching-between-modes) (e.g., via the [`switch_mode`](/advanced-usage/available-tools/switch-mode) tool).<br />- This field is **not displayed in the mode selector UI** - that's handled by the `description` field. |
| Custom Instructions (optional) (`customInstructions`) | **Specific behavioral guidelines** or rules for the mode.<br />- These instructions are added near the end of the system prompt to further refine ADTEC Code's behavior beyond the `roleDefinition`.<br />- This can be provided directly in the configuration or via separate instruction files.                                                                                                                               |
...
**Before**: Manually recreating custom modes and copying `.adtec/rules-{slug}/` folders for each new project or team member. Changing slugs required manual path updates in the YAML file.
...
ADTEC Code packages the mode's configuration and any rules found in the project's `.adtec/rules-{slug}/` directory into the YAML file.
...
3.  Choose the import level in the dialog that appears:
    ...
    -   **Project**: The mode is available only in the current workspace. It's added to the `.adtecmodes` file, and its rules are saved to the `.adtec/rules-{slug}/` directory within the project.
    -   **Global**: The mode is available in all your projects. It's added to your global settings, and its rules are stored in your system's global ADTEC Code configuration directory (e.g., `~/.adtec/rules-{slug}/`).
...
- **Global** modes are available across all your VS Code projects. **Project** modes are specific to the workspace where they are imported and are stored in a `.adtecmodes` file at the root of your project.
...
- The rules are still preserved. They are stored in a global `rules-{slug}` folder in your user home directory (e.g., `~/.adtec/rules-my-custom-mode/`) instead of the project-specific `.adtec` folder.
...
To configure modes, open the ADTEC Code panel, click the Mode menu under the chatbox, then click the <Codicon name="settings-gear" />. From there, use "Edit Global Modes" to open settings/custom_modes.yaml, or "Edit Project Modes (.adtecmodes)" to open your workspace-level configuration.

### 1. Ask ADTEC Code! (Recommended)

You can quickly create a basic custom mode by asking ADTEC Code to do it for you. For example:
```
Create a new mode called "Documentation Writer". It should only be able to read files and write Markdown files.
```
ADTEC Code will guide you through the process, prompting for necessary information for the properties described in the [What's Included in a Custom Mode?](#whats-included-in-a-custom-mode) table. ADTEC Code will create the mode using the preferred YAML format. For fine-tuning or making specific adjustments later, you can use the Modes page or manual configuration.

### 2. Using the Modes Page

1.  **Open the Modes page:** Open the ADTEC Code panel, click the Mode menu under the chatbox, then click the <Codicon name="settings-gear" />.
...
<img src="/img/custom-modes/custom-modes-4.png" alt="Custom mode creation interface on the Modes page" width="600" />
    ...
    *The custom mode creation interface showing fields for name, slug, description, save location, role definition, available tools, custom instructions.*
    ...
    The interface provides fields for `Name`, `Slug`, `Description`, `Save Location`, `Role Definition`, `When to Use (optional)`, `Available Tools`, and `Custom Instructions`. After filling these, click the "Create Mode" button. ADTEC Code will save the new mode in YAML format.

*Refer to the [What's Included in a Custom Mode?](#whats-included-in-a-custom-mode) table for conceptual explanations of each property. File type restrictions for the "edit" tool group can be added by asking ADTEC Code or through manual YAML/JSON configuration.*

### 3. Manual Configuration (YAML & JSON)

You can directly edit the configuration files to create or modify custom modes. This method offers the most control over all properties. ADTEC Code now supports both YAML (preferred) and JSON formats.
...
*   **Project Modes:** Edit the `.adtecmodes` file (which can be YAML or JSON) in your project root. From the Modes page, click "Edit Project Modes (.adtecmodes)" to open or create the workspace file.

These files define an array/list of custom modes.

**YAML Example (`custom_modes.yaml` or `.adtecmodes`):**
```yaml
customModes:
  - slug: docs-writer
...
**JSON Alternative (`custom_modes.json` or `.adtecmodes`):**
```json
{
  "customModes": [
    {
...
*   **Usage:** Used internally and in file/directory names for mode-specific rules (e.g., `.adtec/rules-{slug}/`).
...
##### `name`
*   **Purpose:** The display name shown in the ADTEC Code UI.
...
##### `whenToUse`
*   **Purpose:** (Optional) Provides guidance for ADTEC Code's automated decision-making, particularly for mode selection and task orchestration.
*   **Format:** A string describing ideal scenarios or task types for this mode.
*   **Usage:** This field is used by ADTEC Code for automated decisions and is **not displayed in the mode selector UI** - that's handled by the `description` field. If populated, ADTEC Code uses this description for orchestration and mode switching; otherwise, the first sentence of `roleDefinition` is used.
...
While JSON is still fully supported and won't be deprecated, new modes created via the UI or by asking ADTEC Code will default to YAML. Both `.adtecmodes` files and global configuration files can be either YAML or JSON format.
...
*   **Global Modes:** The automatic migration from `custom_modes.json` to `custom_modes.yaml` happens only for global modes when ADTEC Code starts up, under these conditions:
    1.  ADTEC Code starts up.
...
*   **Project Modes (`.adtecmodes`):**
    *   **No automatic startup migration:** Unlike global modes, project-specific `.adtecmodes` files are not automatically converted from JSON to YAML when ADTEC Code starts. Manual conversion is required for existing JSON `.adtecmodes` files.
    *   **Format Detection:** ADTEC Code can read `.adtecmodes` files in either YAML or JSON format. ADTEC Code automatically detects the format of `.adtecmodes` files by attempting to parse them as YAML first.
    *   **Conversion on UI Edit:** If you edit a project-specific mode through the ADTEC Code UI (e.g., via the Modes page), and the existing `.adtecmodes` file is in JSON format, ADTEC Code will save the changes in YAML format. This effectively converts the file to YAML. The original JSON content will be overwritten with YAML.
    *   **Manual Conversion:** If you want to convert an existing `.adtecmodes` JSON file to YAML without making UI edits, you'll need to do this manually. You can:
        1.  Open your existing JSON `.adtecmodes` file.
        2.  Convert its content to YAML (you can ask ADTEC Code to help with this, or use an online converter).
        3.  Replace the content of your `.adtecmodes` file with the new YAML content, or rename the old file (e.g., `.adtecmodes.json.bak`) and save the new content into a file named `.adtecmodes`.
        Ensure the resulting YAML is valid.

:::tip
For manual conversions of `.adtecmodes` files, you can use online JSON to YAML converters or ask ADTEC Code to help reformat a specific mode configuration from JSON to YAML. Always validate your YAML before saving.
:::
---
...
**Preferred Method: Directory (`.adtec/rules-{mode-slug}/`)**
```
.
├── .adtec/
...
**Fallback Method: Single File (`.adtecrules-{mode-slug}`)**
```
.
├── .adtecrules-docs-writer  # Example for mode slug "docs-writer"
└── ... (other project files)
```

**Legacy Fallback**
For backward compatibility, the system also checks the legacy mode-specific rules filename as an additional fallback. New projects should use the current ADTEC rules filename instead.

The directory method takes precedence if it exists and contains files.

**Rules Directory Scope:**
- **Global modes:** Rules are stored in `~/.adtec/rules-{slug}/` (note the trailing slash)
- **Project modes:** Rules are stored in `{workspace}/.adtec/rules-{slug}/` (note the trailing slash)
:::
...
There are two ways ADTEC Code loads these instructions, with a clear preference for the newer directory-based method:

**1. Preferred Method: Directory-Based Instructions (`.adtec/rules-{mode-slug}/`)**

*   **Structure:** Create a directory named `.adtec/rules-{mode-slug}/` in your workspace root. Replace `{mode-slug}` with your mode's slug (e.g., `.adtec/rules-docs-writer/`).
*   **Content:** Place one or more files (e.g., `.md`, `.txt`) containing your instructions inside this directory. You can organize instructions further using subdirectories. Files within the `.adtec/rules-{mode-slug}/` directory are read recursively and appended in alphabetical order based on filename (case-insensitive).
...
**2. Fallback (Backward Compatibility): File-Based Instructions (`.adtecrules-{mode-slug}`)**

*   **Structure:** If the `.adtec/rules-{mode-slug}/` directory **does not exist or is empty**, ADTEC Code will look for a single file named `.adtecrules-{mode-slug}` in your workspace root (e.g., `.adtecrules-docs-writer`).
*   **Loading:** If found, the content of this single file will be loaded as instructions for the mode.

**Precedence:**

*   The **directory-based method (`.adtec/rules-{mode-slug}/`) takes precedence**. If this directory exists and contains files, any corresponding root-level `.adtecrules-{mode-slug}` file will be **ignored** for that mode.
...
1. Project-level mode configurations (from `.adtecmodes` - YAML or JSON)
2. Global mode configurations (from `custom_modes.yaml`, then `custom_modes.json` if YAML not found)
3. Default mode configurations

**Important:** When modes with the same slug exist in both `.adtecmodes` and global settings, the `.adtecmodes` version completely overrides the global one. This applies to ALL properties, not just some. For instance, if you have a global 'code' mode and a project-specific 'code' mode in `.adtecmodes`, the project version will be used when working in that project, and all properties from the global version are ignored.
...
*   **Note on Instruction Files:** Within the loading of mode-specific instructions from the filesystem, the directory `.adtec/rules-{mode-slug}/` takes precedence over the single file `.adtecrules-{mode-slug}` found in the workspace root.

---

## Overriding Default Modes

You can override ADTEC Code's built-in modes (like `💻 Code`, `🪲 Debug`, `❓ Ask`, `🏗️ Architect`, `🪃 Orchestrator`) with customized versions. This is done by creating a custom mode with the same slug as a default mode (e.g., `code`, `debug`).
...
1. **Open the Modes page:** Open the ADTEC Code panel, click the Mode menu under the chatbox, then click the <Codicon name="settings-gear" />.
2. **Edit Project Modes (.adtecmodes):** Click "Edit Project Modes (.adtecmodes)" to open or create the workspace file.
3. **Add Your Override:**
...
:::tip
**Let ADTEC Code Build Your Regex Patterns**

Instead of writing complex regex manually, ask ADTEC Code:
```
Create a regex pattern that matches JavaScript files but excludes test files
```
ADTEC Code will generate the pattern. Remember to adapt it for YAML (usually single backslashes) or JSON (double backslashes).
:::
...
When deleting a mode through the UI, ADTEC Code will prompt you about deleting the associated rules folder, showing the exact path before deletion. This helps prevent accidental loss of custom instructions.

### Global Rules Directory
In addition to mode-specific rules directories, there's also a generic `.adtec/rules/` directory (without mode suffix) that can be used for shared rules across all modes.
