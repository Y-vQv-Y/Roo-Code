---
description: Learn how to create and use custom slash commands in ADTEC Code to automate workflows and extend functionality with markdown-based definitions.
keywords:
  - slash commands
  - custom commands
  - ADTEC Code commands
...
Create custom slash commands to automate repetitive tasks and extend ADTEC Code's functionality with simple markdown files.

:::tip Quick Start
Type `/` in chat to select a command. To create or manage commands, open Settings > Slash Commands. You can still store commands in `.adtec/commands/` (project) or `~/.adtec/commands/` (global).
:::
...
Custom commands extend ADTEC Code's functionality by adding markdown files to specific directories:

- **Project-specific**: `.adtec/commands/` in your workspace root
- **Global**: `~/.adtec/commands/` in your home directory
...
- **`mode`**: (Optional) Mode slug to switch to before running the command (e.g., `code`, `architect`). ADTEC Code switches to this mode first, then executes the command content in that mode's context
...
1. Click the gear icon in ADTEC Code and open Settings
...
When you run this command, ADTEC Code switches to Architect mode first, then processes the command content with Architect's role definition and tool restrictions.
...
ADTEC Code includes powerful built-in commands that provide specialized functionality:
...
- Generates mode-specific `AGENTS.md` files in `.adtec/rules-*` directories
...
- **Check file location**: Ensure custom command files are in `.adtec/commands/` or `~/.adtec/commands/`
...
**Command Conflicts:**
- Project commands (`.adtec/commands/`) override global commands (`~/.adtec/commands/`) with the same name
...
**File System Errors:**
- **Permission Issues**: Ensure you have write permissions to the `.adtec/commands/` directory
...
- [Using Modes](/basic-usage/using-modes) - Learn about ADTEC Code's different operational modes
- [Custom Instructions](/features/custom-instructions) - Set persistent instructions for ADTEC Code
