---
description: Learn how to use checkpoints in ADTEC Code to save your progress, revert changes, and manage your coding workflow with confidence.
keywords:
  - checkpoints
  - save progress
  - version control
...
Checkpoints automatically version your workspace files during ADTEC Code tasks, enabling non-destructive exploration of AI suggestions and easy recovery from unwanted changes.
...
Access checkpoint settings in ADTEC Code settings under the "Checkpoints" section:
...
2. Configure checkpoint behavior:
   ...
   <img src="/img/checkpoints/checkpoints.png" alt="Checkpoint settings in ADTEC Code configuration" width="500" />
...
- **Scope**: Checkpoints only capture changes made during active ADTEC Code tasks
...
When restoration executes, ADTEC Code:
...
#### .adtecignore Behavior

The `.adtecignore` file (which controls AI access to files) is separate from checkpoint tracking:
- Files excluded by `.adtecignore` but not by `.gitignore` will still be checkpointed
- Changes to AI-inaccessible files can still be restored through checkpoints

This separation is intentional, as `.adtecignore` limits which files the AI can access, not which files should be tracked for version history.
