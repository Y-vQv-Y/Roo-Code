---
description: Use Git worktrees to work on multiple branches simultaneously with ADTEC Code, each in its own VS Code window.
keywords:
  - worktrees
  - git worktrees
  - multiple branches
...
Git worktrees allow you to work on multiple branches of the same repository simultaneously by keeping them in different directories. Each worktree gets its own VS Code window with ADTEC Code, enabling parallel development without branch switching.
...
1. Open the ADTEC Code settings by clicking the gear icon
...
- **Organized locations**: Use a consistent location like `~/.adtec/worktrees/` for all your worktrees
