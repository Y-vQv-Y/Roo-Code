---
description: Learn how Codebase Indexing helps ADTEC Code understand large projects using AI embeddings and semantic search. Setup guide for OpenAI and Qdrant integration.
keywords:
  - codebase indexing
  - semantic search
  - AI embeddings
...
Codebase Indexing transforms how ADTEC Code understands your project by creating a semantic search index using AI embeddings. Instead of searching for exact text matches, it understands the *meaning* of your queries, helping ADTEC Code find relevant code even when you don't know specific function names or file locations.
...
4. **Provides the [`codebase_search`](/advanced-usage/available-tools/codebase-search) tool** to ADTEC Code for intelligent code discovery
...
2. In ADTEC Code settings:
   - Provider: **Google Gemini**
   - API Key: Your Google AI Studio key

:::info Other Providers Available
While this guide focuses on Google Gemini since it's currently free, ADTEC Code also supports OpenAI, Ollama, OpenAI-compatible, Mistral, Vercel AI Gateway, Bedrock, and OpenRouter providers. You can explore these options in the configuration dropdown.
:::
...
You can monitor the status and manage all configuration for the codebase indexer directly from the ADTEC Code chat interface.
...
- **Enhanced AI Understanding**: ADTEC Code can better comprehend and work with your codebase
...
- Files matching `.adtecignore` patterns
...
- Fix:
  ...
  - ADTEC Code will display a clear validation message if the key is invalid
...
- Consider adding patterns to `.adtecignore`

---

## Using the Search Feature

Once indexed, ADTEC Code can use the [`codebase_search`](/advanced-usage/available-tools/codebase-search) tool:
