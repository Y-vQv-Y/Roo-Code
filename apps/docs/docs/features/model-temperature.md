---
description: Learn how to adjust model temperature in ADTEC Code to optimize AI responses for different tasks, from precise code generation to creative brainstorming.
keywords:
    - model temperature
    - AI settings
    - output randomness
...
## Default Values in ADTEC Code

ADTEC Code uses a default temperature of 0.0 for most models, optimizing for maximum determinism and precision in code generation. This applies to OpenAI models, Anthropic models (non-thinking variants), LM Studio models, and most other providers.
...
Some specialized models don't support temperature adjustments at all, in which case ADTEC Code respects these limitations automatically.
...
1.  **Open the ADTEC Code Panel:** Click the ADTEC Code icon (<KangarooIcon />) in the VS Code Activity Bar
...
5.  **Set Your Value:** Adjust the slider to your preferred value
    ...
    <img src="/img/model-temperature/model-temperature.png" alt="Temperature setting in ADTEC Code settings panel" width="550" />
    *Temperature slider in ADTEC Code settings panel*
...
ADTEC Code implements temperature handling with these considerations:
...
1. **Start with defaults** - Begin with ADTEC Code's preset values (0.0 for most tasks) as your baseline
...
- Works with all [API providers](/providers/openai) supported by ADTEC Code
