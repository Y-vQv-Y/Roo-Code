#!/bin/bash
# ADTEC Code CLI Local Build Script
#
# Usage:
...
create_tarball() {
    ...
    RELEASE_DIR="$REPO_ROOT/adtec-code-cli-${PLATFORM}"
    ...
    cat > "$RELEASE_DIR/bin/adtec-code" << 'WRAPPER_EOF'
#!/usr/bin/env node
...
process.env.ADTEC_CODE_CLI_ROOT = join(__dirname, '..');
process.env.ADTEC_CODE_EXTENSION_PATH = join(__dirname, '..', 'extension');
const ripgrepPath = join(__dirname, 'rg');
if (existsSync(ripgrepPath)) {
  process.env.ADTEC_CODE_RIPGREP_PATH = ripgrepPath;
}
...
WRAPPER_EOF
    ...
    chmod +x "$RELEASE_DIR/bin/adtec-code"
...
verify_local_install() {
    ...
    ADTEC_CODE_LOCAL_TARBALL="$TARBALL_PATH" \
    ADTEC_CODE_INSTALL_DIR="$VERIFY_INSTALL_DIR" \
    ADTEC_CODE_BIN_DIR="$VERIFY_BIN_DIR" \
    ADTEC_CODE_VERSION="$VERSION" \
    "$CLI_DIR/install.sh" || {
        rm -rf "$VERIFY_DIR"
    ...
    # Test --help
    if ! "$VERIFY_BIN_DIR/adtec-code" --help > /dev/null 2>&1; then
        rm -rf "$VERIFY_DIR"
    ...
    # Test --version
    if ! "$VERIFY_BIN_DIR/adtec-code" --version > /dev/null 2>&1; then
        rm -rf "$VERIFY_DIR"
...
print_summary() {
    ...
    if [ "$LOCAL_INSTALL" = true ]; then
        echo "  Installed to: ~/.adtec/cli"
        echo "  Binary: ~/.local/bin/adtec-code"
        echo ""
        echo "  Test it out:"
        ...
        echo "    ADTEC_CODE_LOCAL_TARBALL=$REPO_ROOT/$TARBALL ./apps/cli/install.sh"
        echo ""
...
main() {
    ...
    printf "${BLUE}${BOLD}"
    echo "  ╭─────────────────────────────────╮"
    echo "  │   ADTEC Code CLI Local Build      │"
    echo "  ╰─────────────────────────────────╯"
    printf "${NC}"
    echo ""
