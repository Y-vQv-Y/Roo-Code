// pnpm --filter @adtec-code/cli test src/agent/__tests__/extension-host.test.ts
...
vi.mock("@/lib/storage/index.js", () => ({
	createEphemeralStorageDir: vi.fn(() => Promise.resolve("/tmp/adtec-code-cli-test-ephemeral")),
}))
...
describe("ExtensionHost", () => {
	const initialRooCliRuntimeEnv = process.env.ADTEC_CODE_CLI_RUNTIME
	...
	beforeEach(() => {
		...
		if (initialRooCliRuntimeEnv === undefined) {
			...
			process.env.ADTEC_CODE_CLI_RUNTIME = initialRooCliRuntimeEnv
		}
		// Clean up globals
	...
	describe("constructor", () => {
		...
		it("should mark process as CLI runtime", () => {
			delete process.env.ADTEC_CODE_CLI_RUNTIME
			createTestHost()
			expect(process.env.ADTEC_CODE_CLI_RUNTIME).toBe("1")
		})
	...
	describe("dispose", () => {
		...
		it("should clear ADTEC_CODE_CLI_RUNTIME on dispose when it was previously unset", async () => {
			delete process.env.ADTEC_CODE_CLI_RUNTIME
			host = createTestHost()
			expect(process.env.ADTEC_CODE_CLI_RUNTIME).toBe("1")
			...
			expect(process.env.ADTEC_CODE_CLI_RUNTIME).toBeUndefined()
		})

		it("should restore prior ADTEC_CODE_CLI_RUNTIME value on dispose", async () => {
			process.env.ADTEC_CODE_CLI_RUNTIME = "preexisting-value"
			host = createTestHost()
			expect(process.env.ADTEC_CODE_CLI_RUNTIME).toBe("1")

			await host.dispose()

			expect(process.env.ADTEC_CODE_CLI_RUNTIME).toBe("preexisting-value")
		})
	})
	...
	describe("ephemeral mode", () => {
		...
		it("should clean up ephemeral storage directory on dispose", async () => {
			...
			const mockEphemeralDir = "/tmp/adtec-code-cli-test-ephemeral-cleanup"
		...
		it("should handle ephemeral storage cleanup errors gracefully", async () => {
			...
			// Set up a mock ephemeral storage directory
			setPrivate(host, "ephemeralStorageDir", "/tmp/adtec-code-cli-test-ephemeral-error")
